...
for i in input_data:
    ...
    position1 = int(str(password_policy).split("-")[0]) - 1
    position2 = int(str(password_policy).split("-")[1].split(" ")[0]) - 1
    given_letter = str(password_policy).split(" ")[1]
    if password[position1] == password[position2]:
        continue
    elif password[position1] == given_letter or password[position2] == given_letter:
        valid_passwords += 1
